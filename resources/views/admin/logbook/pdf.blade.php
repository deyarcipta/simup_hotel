...
<head>
    <meta charset="UTF-8">
    <title>Laporan Logbook Bulanan UP Laundry</title>
    <style>
        body {
...
<body>
    ...
    <div class="header">
        <h2>Laporan Logbook Harian Unit Produksi (UP) Laundry</h2>
    ...
    @php
        ...
        $totalCols = 9 + (3 * $jumlahShiftSetting);
    @endphp
        ...
        <thead>
            <tr>
                ...
                @if($jumlahShiftSetting == 3)
                    <th colspan="3">Omzet Shift 3 Malam</th>
                @endif
                <th rowspan="2" style="width: 12%;">Total Omzet</th>
                <th rowspan="2" style="width: 7%;">Detergen</th>
        ...
        <tbody>
            ...
            @forelse($logbooks as $logbook)
                ...
                <tr>
                    ...
                    {{-- Shift 1 --}}
                    <td class="text-center">{{ $s1 ? number_format($s1->jumlah_kiloan, 1) : 0 }}</td>
                    <td class="text-center">{{ $s1?->jumlah_satuan ?? 0 }}</td>
                    <td class="text-center">{{ $s1?->jumlah_dry_cleaning ?? 0 }}</td>
                    
                    {{-- Shift 2 --}}
                    @if($jumlahShiftSetting >= 2)
                        <td class="text-center">{{ $s2 ? number_format($s2->jumlah_kiloan, 1) : 0 }}</td>
                        <td class="text-center">{{ $s2?->jumlah_satuan ?? 0 }}</td>
                        <td class="text-center">{{ $s2?->jumlah_dry_cleaning ?? 0 }}</td>
                    @endif

                    {{-- Shift 3 --}}
                    @if($jumlahShiftSetting == 3)
                        <td class="text-center">{{ $s3 ? number_format($s3->jumlah_kiloan, 1) : 0 }}</td>
                        <td class="text-center">{{ $s3?->jumlah_satuan ?? 0 }}</td>
                        <td class="text-center">{{ $s3?->jumlah_dry_cleaning ?? 0 }}</td>
                    @endif
                    
                    <td class="text-right fw-bold" style="color: #0d6efd;">Rp {{ number_format($totalHarian, 0, ',', '.') }}</td>
                    <td class="text-center">{{ $logbook->stok_detergen ?? '-' }}</td>
                    <td>{{ $logbook->status_mesin ?? '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="{{ $totalCols }}" class="text-center py-4">Tidak ada data logbook untuk periode ini.</td>
                </tr>
            @endforelse
        </tbody>
        @if($logbooks->count() > 0)
            <tfoot>
                <tr style="background-color: #e9e9e9; font-weight: bold;">
                    ...
                    <td colspan="{{ 3 * $jumlahShiftSetting }}" class="text-center">-</td>
    ...
    <div class="signature-section">
        <table class="signature-table">
            <tr>
                <td>
                    Menyetujui,<br>
                    <strong>Kepala Program Keahlian Perhotelan</strong>
                    <div class="signature-space"></div>
                    ( ............................................ )
                </td>
                <td>
                    Jakarta, {{ \Carbon\Carbon::now()->translatedFormat('d F Y') }}<br>
                    <strong>Pembina Unit Produksi Laundry</strong>
                    <div class="signature-space"></div>
                    ( ............................................ )
                </td>
            </tr>
